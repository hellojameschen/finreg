...
def get_best(df):
    ...
    return df[['original_org_name', 'comment_org_name', 'best_match_score', 'best_match_name']]

df = get_best(pd.read_csv(BASE_DIR + file)).groupby('comment_org_name').first()
